    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mBuilder.orientation == LinearLayoutManager.VERTICAL) {
                left = child.getLeft() - params.leftMargin + transitionX +
                        ScreenUtil.dp2px(context, mBuilder.leftMargin);
                top = child.getBottom() + params.bottomMargin;
                right = child.getRight() + params.rightMargin -
                        ScreenUtil.dp2px(context, mBuilder.rightMargin);
                bottom = top + mBuilder.dividerHeight + transitionY;
            } else {
                top = child.getTop() - params.topMargin +
                        ScreenUtil.dp2px(context, mBuilder.topMargin);
                bottom = child.getBottom() + params.bottomMargin + transitionY -
                        ScreenUtil.dp2px(context, mBuilder.bottomMargin);
