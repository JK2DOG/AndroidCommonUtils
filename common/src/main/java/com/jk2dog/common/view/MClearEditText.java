...
public class MClearEditText extends AppCompatEditText implements View.OnFocusChangeListener, TextWatcher {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(dip2px(context, 2), this.getHeight() - 1,
                this.getWidth() - dip2px(context, 2),
    ...
    private void updateDrawable(boolean hasFocus) {
        if (length() > 0 && hasFocus) {
            setCompoundDrawables(mLeftDrawable,
                    getCompoundDrawables()[DRAWABLE_TOP], mClearDrawable,
                    getCompoundDrawables()[DRAWABLE_BOTTOM]);
        } else {
            setCompoundDrawables(mLeftDrawable,
                    getCompoundDrawables()[DRAWABLE_TOP], null,
                    getCompoundDrawables()[DRAWABLE_BOTTOM]);
        }
    }
